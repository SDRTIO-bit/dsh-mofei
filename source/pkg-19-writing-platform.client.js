  ...
  apply(ctx) {
    ...
    function Workspace() {
      ...
      function arm(kind, id) {
        ...
        setArmed({ kind, id })
        timer.timeout(() => setArmed((current) => current && current.kind === kind && current.id === id ? null : current), 4000)
        return false
      }
