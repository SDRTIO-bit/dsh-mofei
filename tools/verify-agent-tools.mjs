...
try {
  ...
  if (!parsed) {
    ...
    if ( total === 72) ok('agent 可见墨扉工具总数 = 72')
    else fail(`agent 可见墨扉工具总数应为 72，实际 ${total}`)
    if (mofei === 36) ok('mofei_* = 36')
    else fail(`mofei_* 应为 36，实际 ${mofei}`)
    if (openfic === 36) ok('openfic_* = 36')
    else fail(`openfic_* 应为 36，实际 ${openfic}`)
