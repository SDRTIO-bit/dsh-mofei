...
> **Mofei fork (v0.1.1-mofei.1)** — this copy vendors the upstream package and adds two
> per-call parameters on top of `model` / `provider`:
> - `effort`: `off | high | max` — child reasoning strength, forwarded as
>   `agentOptions.reasoningEffort` (DeepSeek wire `reasoning_effort`).
> - `context`: free-text context assembled by the calling agent, prefixed to the
>   child's prompt (`【主模型装配的上下文】…【任务】…`).
>
> Installed as a `link:` dependency of the `novel` DSH profile (3088) and exposed to the
> `mofei-writer` agent preset only (Mofei writing sessions). See
> `OpenFic-DSH/PLAN-v0.16-subagent-max.md` for the integration plan.
