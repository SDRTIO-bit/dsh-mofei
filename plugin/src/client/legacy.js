...
export function createClient(require) {
  ...
  // 统一的 SVG 图标集（Lucide 风格描边，stroke=currentColor 随主题着色，避免 emoji 拼凑感）
  const MF_ICONS = {
  ...
  function Workspace(props) {
    ...
    function loadRetrievalStatus() { setRetrievalBusy(true); call('retrieval-model-status').then((result) => setRetrievalStatus(result || null)).catch((failure) => setRetrievalStatus({ embeddingReady: false, rerankReady: false, embeddingError: String((failure && failure.message) || failure) })).finally(() => setRetrievalBusy(false)) }
      function loadRagIndexStatus() { if (!projectId) { setRagIndexStatus(null); return } call('rag-status', { projectId }).then((result) => setRagIndexStatus(result || null)).catch(() => setRagIndexStatus(null)) }
     function openSettingsPanel() { if (props && props.onOpenSettings) props.onOpenSettings(); setSettingsOpen(true); if (!retrievalStatus) loadRetrievalStatus(); loadRagIndexStatus() }
  ...
  h('button', { className: 'mf-act' + (tab === 'trash' ? ' on' : ''), type: 'button', title: '回收站（已删除实体，可恢复）', onClick: () => { setTab('trash'); loadTrash() } }, mfIcon('trash'), h('span', null, '回收站')),
  ...
  [['projects', 'grid', '项目'], ['retrieve', 'search', '检索'], ['characters', 'user', '角色'], ['world', 'globe', '世界'], ['notes', 'file', '笔记'], ['trash', 'trash', '回收站']].map((item) => h('button', { key: item[0], type: 'button', className: tab === item[0] ? 'on' : '', onClick: () => { setTab(item[0]); if (item[0] === 'trash') loadTrash() } }, h('span', { className: 'ic' }, mfIcon(item[1])), item[2])),
  h('button', { type: 'button', className: skillsOpen ? 'on' : '', title: '写作指令与工作流', onClick: openWritingSkills }, h('span', { className: 'ic' }, '✦'), '技能')
        ...
        h('main', { className: 'mf-editor' },
          ...
          : h('div', { className: 'mf-editor-pane' },
            ...
            chapter ? h('div', { className: 'mf-code' },
